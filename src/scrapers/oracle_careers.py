"""Oracle scraper adapter — Oracle HCM REST API (same platform as JPMorgan)."""
import logging
...
logger = logging.getLogger("jobsearch.scrapers.oracle_careers")
...
class OracleScraper(BaseScraper):
    """Scrapes Oracle careers via Oracle HCM REST API."""
    ...
    def fetch_jobs(self) -> list[RawJob]:
        ...
        for page in range(MAX_PAGES):
            ...
            logger.info("Fetching Oracle page %d: offset=%d", page + 1, offset)
        ...
        logger.info("Oracle scraper found %d jobs total", len(all_jobs))
        return all_jobs


register_adapter("oracle", OracleScraper)
